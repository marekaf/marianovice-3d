...
assert(report.mapSVG.includes('BEZ SCHODŮ'),'The east terrace explicitly has no stairs');
...
for(const id of ['sauna','greenhouse','westTerrace','pergola','service','gate','wicket','drainage'])assert(find(id),`${id} has a surface-specific height control`);
assert.equal(levels.filter(m=>m[2]==='finish').length,13,'All finish heights are on the plan');
assert(!report.html.includes('level-schedule'),'The drawing does not use a separate finish dashboard');
...
for(const match of report.mapSVG.matchAll(/<g data-elevation="([^"]+)"[\s\S]*?<\/g>/g)){
  if(match[1]==='house')continue;
  assert.equal((match[0].match(/<text[^>]*font-size="9"/g)??[]).length,2,`${match[1]} uses the reference two-row elevation box`);
...
for(const id of ['A','carport','westTerrace','house','eastTerrace'])assert(report.exportSVG.includes(`data-elevation="${id}"`),`${id} finish control survives SVG export`);
