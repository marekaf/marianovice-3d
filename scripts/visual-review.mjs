...
await writeFile(join(output,'manifest.json'),JSON.stringify({floorFinish:floorReference?.metadata??{mode:'procedural-placeholder'},checks:failed?'failed':'passed',images},null,2)+'\n');
...
<p>${floorReference?'Private Floorify Champagne reference. Keep these images local.':'Generic procedural floor placeholder, NOT the selected Floorify Champagne decor.'} No Unreal rendering.</p>
