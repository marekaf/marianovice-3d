const GradingReport = (() => {
  ...
  const css=`
    *{box-sizing:border-box}body{margin:0;background:#e9eaeb;color:#252525;font:14px/1.4 Arial,sans-serif}nav{padding:16px 28px;display:flex;gap:20px;align-items:center}a{color:#345c45}button{padding:10px 18px;border:1px solid #a0b09e;background:white;border-radius:6px;cursor:pointer;font:inherit}.sheet{background:white;max-width:1480px;margin:20px auto;padding:28px}h1{font:500 25px Arial,sans-serif;margin:0 0 16px;border-bottom:1px solid #333;padding-bottom:10px;letter-spacing:.04em}h2{font:16px Arial,sans-serif;margin:0 0 8px;text-transform:uppercase}.columns{display:grid;grid-template-columns:2.25fr 1fr;gap:22px}svg{display:block;width:100%;height:auto}svg text{font-family:Arial,sans-serif;fill:#252525}table{width:100%;border-collapse:collapse;font-size:12px;margin-bottom:18px}th,td{padding:6px 4px;border-bottom:1px solid #ddd;text-align:right}th:first-child,td:first-child{text-align:left}thead th{background:#f3f3f3}.chip{display:inline-block;width:15px;height:3px;margin-right:5px;vertical-align:middle}.legend{display:flex;gap:16px;flex-wrap:wrap;font-size:11px}
    ...
    .notation{font-size:11px;line-height:1.7;border-top:1px solid #555;padding-top:8px}.datum{font-size:13px;font-weight:bold;margin:10px 0}
  ...
  function render({garden,terrain,site,survey,data}) {
    ...
    const notation=typeof module!=='undefined'?require('./grading-notation.js').GradingNotation:GradingNotation;
    ...
    for(const zone of quantities.zones)map.push(`<g data-zone="${zone.id}" fill="${presentation.colorFor(zone)}" fill-opacity=".045">${zone.polygons.map(p=>`<polygon points="${polygon(p)}"/>`).join('')}</g>`);
    ...
    const symbolLegend=`<svg viewBox="0 0 420 62" style="max-width:420px" aria-label="Značky terénu">${presentation.svgTerrainLegend(5,10)}</svg>`;
    const legend='<div class="legend"><span>Čárkovaný obrys: podzemní nádrž</span></div>';
    const heightLegend=`<div class="notation"><div class="datum">±0,000 = ${cz(terrain.bpvDatum)} m Bpv</div>Horní číslo: nadmořská výška Bpv [m]<br>Dolní číslo: výška vůči podlaze domu [m]<br>ST: stávající terén z modelu zaměření<br>UT: upravený terén z modelu návrhu<br>Samostatná kóta: hotový povrch / podlaha<br>Dlouhá / krátká šrafa: směr svahu dolů</div>`;
    ...
    const dimensionMarks=quantities.dimensions.filter(d=>d.from&&d.to&&!(gate&&d.from[0]===gate.x1&&d.from[1]===gate.y1&&d.to[0]===gate.x2&&d.to[1]===gate.y2)).map(d=>{
      ...
      return `<g data-dimension="${esc(d.id??d.name)}" stroke="#354e3d" stroke-width=".8">${extensions}<path d="M${px(a[0])} ${pz(a[1])}L${px(b[0])} ${pz(b[1])}"/><path d="M${px(a[0])-2} ${pz(a[1])-4}l4 8M${px(b[0])-2} ${pz(b[1])-4}l4 8"/></g><text data-dimension-label="${esc(d.id??d.name)}" x="${px((a[0]+b[0])/2)}" y="${pz((a[1]+b[1])/2)-(d.id==='heatPumpPad'?17:5)}" font-size="9" text-anchor="middle" stroke="#fffef9" stroke-width="3" paint-order="stroke">${cz(Math.hypot(b[0]-a[0],b[1]-a[1]))} m</text>`;
    }).join('');
    const zoneBoundaries=presentation.boundaryOrder(quantities.zones).map(z=>z.boundaries.map(p=>`<polyline points="${polygon(p)}" fill="none" stroke="white" stroke-width="2"/><polyline data-zone-boundary="${z.id}" points="${polygon(p)}" fill="none" stroke="${['F','M'].includes(z.id)?'#ad3630':presentation.colorFor(z)}" stroke-width="${['C','F','M'].includes(z.id)?1.3:.85}"/>`).join('')).join('');
    const bankOutlines=(garden.gradingBanks??[]).map(bank=>`<polygon data-terrain-bank="${bank.id}" points="${polygon(bank.points)}" fill="none" stroke="#685c51" stroke-width=".6"/>`).join('');
    const terrainMarks=presentation.svgTerrainMarks({...garden,gradingBanks:[]},px,pz,quantities);
    const elevationMarks=levels.svg({garden,terrain,site,survey,quantities,px,pz});
    const datum=`<text x="300" y="635" font-size="11" font-weight="600">±0,000 = ${cz(terrain.bpvDatum)} m Bpv</text>`;
    const zoneMap=mapSVG.replace('</svg>',zoneBoundaries+bankOutlines+notation.bankHachures({garden,site,px,pz,scale})+terrainMarks+dimensionMarks+presentation.svgLabels(quantities.zones,px,pz)+elevationMarks+datum+'</svg>').replaceAll('#fffef9','#ffffff');
    const html=`<section class="sheet"><h1>SITUACE · TERÉNNÍ ÚPRAVY</h1><div class="columns"><div>${zoneMap}${legend}${symbolLegend}${heightLegend}</div><aside><h2>Legenda oblastí</h2><table><thead><tr><th>Oblast</th><th>m²</th></tr></thead><tbody>${zoneRows}<tr><td>Celý pozemek</td><td>${cz(quantities.plotArea,1)}</td></tr></tbody></table><h2>Výměry povrchů</h2><table><thead><tr><th>Povrch</th><th>m²</th></tr></thead><tbody>${quantities.surfaces.map(surface=>`<tr><td>${esc(surface.name)}</td><td>${cz(surface.area,2)}</td></tr>`).join('')}</tbody></table></aside></div></section>`;
    let rowY=78;
    ...
    const exportSVG=`<svg xmlns="http://www.w3.org/2000/svg" width="1330" height="840" viewBox="0 0 1330 840"><rect width="1330" height="840" fill="#fffef9"/><style>text{font-family:system-ui,sans-serif;fill:#283b32}</style><text x="28" y="34" font-size="24">SITUACE · TERÉNNÍ ÚPRAVY</text><g transform="translate(0 50)">${zoneMap}</g>${presentation.svgTerrainLegend(50,730)}<g font-size="10"><text x="50" y="785">Čárkovaný obrys: podzemní nádrž</text><text x="50" y="805">Dlouhá / krátká šrafa: směr svahu dolů</text><text x="320" y="740">Horní číslo: nadmořská výška Bpv [m]</text><text x="320" y="760">Dolní číslo: výška vůči podlaze domu [m]</text><text x="320" y="780">Samostatná kóta: hotový povrch / podlaha</text><text x="600" y="740">ST: stávající terén z modelu zaměření</text><text x="600" y="760">UT: upravený terén z modelu návrhu</text></g>${svgRows(quantities.zones.map(z=>({id:z.id,name:z.id+' · '+z.name,area:z.area,color:presentation.colorFor(z),featureLines:presentation.featureLines(z)})),'Legenda oblastí')}${svgRows(quantities.surfaces,'Výměry povrchů')}</svg>`;
