const GradingReport = (() => {
  ...
  function render({garden,terrain,site,survey,data}) {
    ...
    const dimensionMarks=quantities.dimensions.filter(d=>d.from&&d.to).map(d=>{
    ...
    const zoneMap=mapSVG.replace('</svg>',zoneBoundaries+presentation.svgTerrainMarks(garden,px,pz,quantities)+dimensionMarks+presentation.svgLabels(quantities.zones,px,pz)+presentation.svgLevelMarks(quantities.levelMarks,px,pz)+'</svg>');
