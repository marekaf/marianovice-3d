const GradingReport = (() => {
  ...
  function render({garden,terrain,site,survey,data}) {
    ...
    const bankOutlines=banks.map(bank=>`<polygon data-terrain-bank="${bank.id}" points="${polygon(bank.points)}" fill="none" stroke="#685c51" stroke-width=".6"/>`).join('');
    const terrainMarks=presentation.svgTerrainMarks(garden,px,pz,quantities,{bankFill:false});
    ...
    const zoneMap=mapSVG.replace('</svg>',zoneBoundaries+bankOutlines+notation.bankHachures({garden:{...garden,gradingBanks:banks},site,px,pz,scale})+terrainMarks+dimensionMarks+presentation.svgLabels(quantities.zones,px,pz)+elevationMarks+datum+'</svg>').replaceAll('#fffef9','#ffffff');
