const GradingReport = (() => {
  ...
  function render({garden,terrain,site,survey,data,baseline}) {
    ...
    const reviewSheet=site.spec.bankReview?.length?`<section class="sheet">${header('Changes & unresolved details',3+Math.ceil(data.sections.length/4))}<div class="balanced"><div><h2>Surface changes and comparison</h2>${baseline?`<p>Before snapshot ${esc(baseline.revision)}. Current snapshot ${rev}. Dashed blue-grey section lines show the previous proposed ground; brown is the survey and green is current proposed ground.</p><table><thead><tr><th>Indicative volume</th><th>Before</th><th>Current</th><th>Difference</th></tr></thead><tbody>${['cut','fill'].map(key=>`<tr><td>${key}</td><td>${f(baseline.totals[key],0)} m³</td><td>${f(t[key],0)} m³</td><td>${signed(t[key]-baseline.totals[key])} m³</td></tr>`).join('')}</tbody></table><p class="muted">Same grid and exclusions. This compares model surfaces, not construction quantities or truck movements.</p>`:'<p>No previous snapshot is available. The sections show current ground and the supplied survey only.</p>'}<h3>Preserved constraints</h3><ul><li>House, terrace, sauna and firepit finish levels.</li><li>Greenhouse, raised-bed and compost footprints; bench position and platform.</li><li>No new stairs; the southern one-metre strip.</li><li>Sampled boundary levels, vehicle gate and wicket landing.</li></ul><h3>What changed</h3><p>The pergola floor is ${f(terrain.houseFFLInternal-site.spec.gatheringPads[0].level-.1)} m below the house. Its 6 × 4 m footprint sits near the north boundary, clear of the bedroom-to-firepit view. The firepit remains in its northeast position. The terrace approach takes a longer route around the planted area, with level connections and broader soil-bank transitions. Lowering alone does not guarantee bedroom privacy. Access, soil stability and drainage still require review before construction.</p>${site.spec.productiveCourt?'<p>The productive court is lowered and regraded into a sloped walking approach with two level crossfall aisles. The greenhouse and each individual raised bed stay level; their bases meet the sloping ground. The former single flat raised-bed platform is not retained.</p>':''}<p>The west and south banks use longer transitions instead of wide flat runouts ending in abrupt slopes. The gathering path has broader shoulders, and the daily dining shoulder blends around fixed platform footprints.</p></div><div><h2>Resolve before construction</h2>${site.spec.bankReview.map((r,i)=>`<div class="warning"><h3>R${i+1} · ${esc(r.label)}</h3><p>${esc(r.status)}</p></div>`).join('')}<p>The amber boxes on the plan identify coordination areas, not designed retaining-wall locations. Wall type, foundations, soil stability, drainage and surface build-ups remain unresolved.</p><p>Residual banks are still steep. The model does not certify safe slopes or constitute a construction-ready grading design.</p></div></div>${footer}</section>`:'';
    const bankSheet=data.reviews.length?`<section class="sheet">${header('Bank review measurements',4+Math.ceil(data.sections.length/4))}<h2>Highest supported model-ground grade</h2><p>R numbers match the amber review boxes on the plan. Coordinates identify where to investigate the current proposal; they are not construction setting-out points.</p><table><thead><tr><th>Review area</th><th>Peak x / z</th><th>Sampled grade</th><th>Supported samples</th></tr></thead><tbody>${data.reviews.map((review,i)=>{
      const region=site.spec.bankReview.find(r=>r.id===review.id);
      return `<tr><td>R${i+1} · ${esc(region.label)}</td><td>${review.peak?`${f(review.peak.x)} / ${f(review.peak.z)}`:'Not available'}</td><td>${review.peak?`${f(review.peak.slope,1)}%`:'Not available'}</td><td>${review.samples}</td></tr>`;
    }).join('')}</tbody></table><p class="muted">${esc(data.metadata.reviewSlopeMethod)} Grid centres are ${f(data.metadata.step)} m apart.</p>${data.reviews.filter(r=>!r.peak).map(r=>`<p class="warning">${esc(site.spec.bankReview.find(region=>region.id===r.id).label)}: No fully supported grade samples in this review area. Additional survey coverage or a closer geometry check is needed.</p>`).join('')}<div class="warning">A numerical grade is not a slope approval. Destination levels, soil stability, retaining structures, drainage and access must be coordinated before terrain work. No retaining-wall or drainage design is supplied by this report.</div>${footer}</section>`:'';
    return {html:sheet1+sheet2+sections.join('')+reviewSheet+bankSheet,css,mapSVG,revision:rev};
  }
  return {render};
})();
