const GradingReport = (() => {
  ...
  function render({garden,terrain,site,survey,data}) {
    ...
    const zoneBoundaries=presentation.boundaryOrder(quantities.zones).map(z=>z.boundaries.map(p=>`<polyline points="${polygon(p)}" fill="none" stroke="white" stroke-opacity="${['F','M'].includes(z.id)?1:0}" stroke-width="2"/><polyline opacity="${['F','M'].includes(z.id)?1:.25}" data-zone-boundary="${z.id}" points="${polygon(p)}" fill="none" stroke="${['F','M'].includes(z.id)?'#e00000':presentation.colorFor(z)}" stroke-width="${['F','M'].includes(z.id)?2:['C'].includes(z.id)?1:.65}"/>`).join('')).join('');
    ...
    const zoneMap=mapSVG.replace('</svg>',zoneBoundaries+terrainMarks+dimensionMarks+presentation.svgLabels(quantities.zones,px,pz,9,{technical:true})+elevationMarks+datum+'</svg>').replaceAll('#fffef9','#ffffff');
