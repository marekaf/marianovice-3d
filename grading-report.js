const GradingReport = (() => {
  ...
  function render({garden,terrain,site,survey,data}) {
    ...
    const heightLegend=`<div class="notation"><div class="datum">±0,000 = ${cz(terrain.bpvDatum)} m Bpv</div>Horní číslo: nadmořská výška Bpv [m]<br>Dolní číslo: výška vůči podlaze domu [m]<br>ST: stávající terén z modelu zaměření · UT: upravený terén z modelu návrhu<br>Samostatná kóta: povrch uvedený u značky<br>Vrstevnice upraveného terénu: 0,50 m · silné: 1,00 m<br>${presentation.terrainLegend.map(([kind,label])=>(kind==='flat'?'═ ':'↘ ')+label).join('<br>')}<br>Čárkovaný obrys: podzemní nádrž</div>`;
    ...
    const finishTable=rows=>`<table><thead><tr><th>Hotový povrch</th><th>Bpv</th><th>±0,00</th></tr></thead><tbody>${rows.map(mark=>`<tr data-finish-level="${mark.id}" data-proposed="${mark.proposed.toFixed(5)}"><td>${mark.zone} · ${mark.label.toLocaleLowerCase('cs')}</td><td>${mark.absoluteText}</td><td>${mark.relativeText}</td></tr>`).join('')}</tbody></table>`;
    ...
    const zoneMap=mapSVG.replace('</svg>',zoneBoundaries+bankOutlines+terrainMarks+dimensionMarks+presentation.svgLabels(quantities.zones,px,pz)+elevationMarks+datum+'</svg>').replaceAll('#fffef9','#ffffff');
