const GradingReport = (() => {
  ...
  function render({garden,terrain,site,survey,data}) {
    ...
    const legend='<div class="legend"><span>Hnědé obdélníky: vyvýšené záhony</span><span>Čárkovaný obrys: podzemní nádrž</span><span>Tmavá přerušovaná čára: zaměřený plot</span><span>Modrý obrys: zakrytý průchod pod chodníkem · modrý bod: podzemní odvodnění</span></div>';
    ...
    const dimensionMarks=quantities.dimensions.filter(d=>d.from&&d.to).map(d=>{const a=d.from.map((v,i)=>v+(d.displayOffset?.[i]??0)),b=d.to.map((v,i)=>v+(d.displayOffset?.[i]??0));return `<g data-dimension="${esc(d.id??d.name)}" stroke="#354e3d" stroke-width=".8"><path d="M${px(a[0])} ${pz(a[1])}L${px(b[0])} ${pz(b[1])}"/><path d="M${px(a[0])-2} ${pz(a[1])-4}l4 8M${px(b[0])-2} ${pz(b[1])-4}l4 8"/></g><text x="${px((a[0]+b[0])/2)}" y="${pz((a[1]+b[1])/2)-(d.id==='heatPumpPad'?17:5)}" font-size="9" text-anchor="middle" stroke="#fffef9" stroke-width="3" paint-order="stroke">${cz(Math.hypot(b[0]-a[0],b[1]-a[1]))} m</text>`;}).join('');
    const zoneBoundaries=presentation.boundaryOrder(quantities.zones).map(z=>z.boundaries.map(p=>`<polyline points="${polygon(p)}" fill="none" stroke="#fffef9" stroke-width="4.2"/><polyline data-zone-boundary="${z.id}" points="${polygon(p)}" fill="none" stroke="${presentation.colorFor(z)}" stroke-width="${z.id==='C'?2.8:2.1}"/>`).join('')).join('');
    ...
    const exportSVG=`<svg xmlns="http://www.w3.org/2000/svg" width="1330" height="840" viewBox="0 0 1330 840"><rect width="1330" height="840" fill="#fffef9"/><style>text{font-family:system-ui,sans-serif;fill:#283b32}</style><text x="28" y="34" font-size="26">Zemní práce · pracovní oblasti</text><g transform="translate(0 50)">${zoneMap}</g>${presentation.svgTerrainLegend(50,730)}<text x="50" y="805" font-size="10">Hnědé obdélníky: vyvýšené záhony · Čárkovaný obrys: podzemní nádrž</text><text x="50" y="823" font-size="10">Tmavá přerušovaná čára: zaměřený plot · Modrý obrys: zakrytý průchod · modrý bod: podzemní odvodnění</text>${svgRows(quantities.zones.map(z=>({name:z.id+' · '+z.name,area:z.area,color:presentation.colorFor(z)})),'Legenda oblastí')}${svgRows(quantities.surfaces,'Výměry povrchů')}<text x="930" y="${rowY}" font-size="16">Výšky vůči podlaze domu ±0,00 m</text>${spots.map((spot,i)=>`<text x="930" y="${rowY+25+i*22}" font-size="11">${spot.id} · ${spot.name}</text><text x="1300" y="${rowY+25+i*22}" text-anchor="end" font-size="11">${cz(spot.existing)} → ${cz(spot.proposed)} m · +${cz(Math.max(0,spot.proposed-spot.existing))} m</text>`).join('')}</svg>`;
