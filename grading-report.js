const GradingReport = (() => {
  ...
  function render({garden,terrain,site,survey,data,baseline}) {
    const rev = revision([garden,site.spec,survey.data.points,terrain.bpvDatum,terrain.houseFFLInternal]);
    ...
    function sectionSVG(section) {
      const values=section.samples.flatMap(s=>[s.existing,s.proposed,s.previous].filter(Number.isFinite).map(bpv)), lo=Math.floor(Math.min(...values)*2)/2-.25,hi=Math.ceil(Math.max(...values)*2)/2+.25;
      ...
      for(const [key,color] of [['existing','#986f49'],['previous','#7982a3'],['proposed','#397351']]) {
        if(key==='previous'&&!baseline)continue;
        for(let i=1;i<section.samples.length;i++) {
          const a=section.samples[i-1],b=section.samples[i];
          out+=`<path d="M${x(a.distance)} ${y(a[key])}L${x(b.distance)} ${y(b[key])}" fill="none" stroke="${color}" stroke-width="2"${key==='previous'?' stroke-dasharray="6 3"':!a.surveyed||!b.surveyed?' stroke-dasharray="3 3"':''} opacity="${a.excluded||b.excluded?'.35':'1'}"/>`;
        }
      }
    ...
    const sections=[];
    for(let first=0;first<data.sections.length;first+=4)sections.push(`<section class="sheet">${header('Ground sections & transitions',3+first/4)}<div class="legend"><span style="color:#986f49">Brown: existing survey interpolation</span><span style="color:#397351">Green: proposed model ground</span><span>Dashed: extrapolated · Faded: building footprint</span>${baseline?'<span style="color:#7982a3">Blue-grey dashed: previous proposal</span>':''}</div><div class="section-grid">${data.sections.slice(first,first+4).map((s,i)=>`<div class="section-card"><h3>S${first+i+1} · ${esc(s.label)}</h3>${sectionSVG(s)}<p class="muted">Length ${f(s.length)} m · Highest sampled ground grade ${f(s.maxSlope,1)}% outside buildings. ${s.id==='fire-pond'?'Includes the pond basin; NOT a pedestrian route. ':''}Ground only, not the walking surface or structural formation. Sampling every ≤${data.metadata.sectionStep} m.</p></div>`).join('')}</div>${footer}</section>`);
    const reviewSheet=site.spec.bankReview?.length?`<section class="sheet">${header('Changes & unresolved details',3+Math.ceil(data.sections.length/4))}<div class="balanced"><div><h2>Before / after the bank pass</h2>${baseline?`<p>Before snapshot ${esc(baseline.revision)}. Current snapshot ${rev}. Dashed blue-grey section lines show the previous proposed ground; brown is the survey and green is current proposed ground.</p><table><thead><tr><th>Indicative volume</th><th>Before</th><th>Current</th><th>Difference</th></tr></thead><tbody>${['cut','fill'].map(key=>`<tr><td>${key}</td><td>${f(baseline.totals[key],0)} m³</td><td>${f(t[key],0)} m³</td><td>${signed(t[key]-baseline.totals[key])} m³</td></tr>`).join('')}</tbody></table><p class="muted">Same grid and exclusions. This compares model surfaces, not construction quantities or truck movements.</p>`:'<p>No previous snapshot is available. The sections show current ground and the supplied survey only.</p>'}<h3>Preserved constraints</h3><ul><li>House, terrace, sauna, pergola and firepit finish levels.</li><li>Greenhouse and raised-bed platforms, bench position and platform.</li><li>Step-free route profiles and the southern one-metre strip.</li><li>Sampled boundary levels, vehicle gate and wicket landing.</li></ul><h3>What changed</h3><p>The west and south banks use longer transitions instead of wide flat runouts ending in abrupt slopes. The gathering path has broader shoulders, and the daily dining shoulder blends around fixed platform footprints.</p></div><div><h2>Resolve before construction</h2>${site.spec.bankReview.map((r,i)=>`<div class="warning"><h3>R${i+1} · ${esc(r.label)}</h3><p>${esc(r.status)}</p></div>`).join('')}<p>The amber boxes on the plan identify coordination areas, not designed retaining-wall locations. Wall type, foundations, soil stability, drainage and surface build-ups remain unresolved.</p><p>Residual banks are still steep. This pass reduces local peaks; it does not certify safe slopes or make the entire model construction-ready.</p></div></div>${footer}</section>`:'';
