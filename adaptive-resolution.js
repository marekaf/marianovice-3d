...
export function createAdaptiveResolution({renderer,full,moving=ratio=>Math.max(1,ratio*.75),settle=160,onRestore=()=>{},setPixelRatio=ratio=>renderer.setPixelRatio(ratio),schedule=setTimeout,cancel=clearTimeout}) {
  let timer=null,reduced=false;
  const target=()=>{const ratio=full();return reduced&&ratio>1?moving(ratio):ratio;};
  const apply=()=>{const ratio=target();if(renderer.getPixelRatio()!==ratio)setPixelRatio(ratio);};
