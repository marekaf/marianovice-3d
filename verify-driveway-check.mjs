...
const sewer=GARDEN.elements.find(e=>e.id==='sewerInspection'),cover=sewer.parts.find(p=>p.kind==='circle');
assert.equal(sewer.meta.nominalDiameterMm,400);
assert.equal(sewer.meta.positionApproximate,true);
assert.equal(cover.r,.2);
assert(renderDrivewayCheckSVG(GARDEN).includes('data-feature="sewerInspection"'));
