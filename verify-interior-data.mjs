...
// Every export in the data file is verified (HOUSE_INTERIOR, HOUSE_LOFT, …); the DXF wall
// map (mm, [[x,y],[x,y]] segments) covers only the first export — the map is the 1.NP block.
// Checks: opening bounds, wall sanity, furniture placement, room perimeter closure (every
// room edge must be covered by a wall or an opening), door-graph reachability from an
// exterior door, and — with the DXF map — that every model wall face lies on a DXF line
...
function verify(data, name, dxfPath) {
  const problems = [];
  ...
  // 1g. furniture (buildFurniture schema) — joinery never enters allWalls; it gets its own
  // placement checks instead: in-room, no 3D overlap with walls/stairs/fireplace/furniture,
  // under the clear height, out of door-egress zones, and not rising into a window
  if (data.furniture) {
    ...
    for (const f of data.furniture) {
      ...
      if (f.modules) {
        const first = Array.isArray(f.front) ? f.front[0] : f.front;
        const runLen = (first === 'E' || first === 'W') ? f.z1 - f.z0 : f.x1 - f.x0;
        const sum = f.modules.reduce((a, b) => a + b, 0);
        if (Math.abs(sum - runLen) > 0.01) note('WARN', `${fid(f)}modules sum ${sum.toFixed(3)} ≠ run length ${runLen.toFixed(3)}`);
      }
      if (f.kind === 'glass' && Math.min(f.x1 - f.x0, f.z1 - f.z0) > 0.06) note('ERR', `${fid(f)}glass panel thicker than 6 cm`);
      const room = data.rooms.find(r => r.id === f.room);
      if (!room) note('ERR', `${fid(f)}room '${f.room}' not found`);
      else if (f.x0 < room.x0 - TOL || f.x1 > room.x1 + TOL || f.z0 < room.z0 - TOL || f.z1 > room.z1 + TOL)
        note('ERR', `${fid(f)}footprint leaves room ${f.room}`);
      if (topOf(f) > (data.clearH ?? 2.52) + TOL) note('ERR', `${fid(f)}top at ${topOf(f).toFixed(2)} m exceeds the clear height`);
      for (const w of allWalls) {
        const r = wallRect(w);
        const ox = Math.min(f.x1, r.x1) - Math.max(f.x0, r.x0);
        const oz = Math.min(f.z1, r.z1) - Math.max(f.z0, r.z0);
        const wh = w.h ?? (data.clearH ?? 2.52);
        if (ox > TOL && oz > TOL && (f.y0 ?? 0) < wh - 0.01) note('ERR', `${fid(f)}overlaps ${wid(w) || 'a wall '}${ox.toFixed(2)}×${oz.toFixed(2)} m`);
      }
      for (const [obj, nm] of [[data.stairs, 'stairs'], [data.fireplace, 'fireplace']]) {
        if (!obj) continue;
        const ox = Math.min(f.x1, obj.x1) - Math.max(f.x0, obj.x0);
        const oz = Math.min(f.z1, obj.z1) - Math.max(f.z0, obj.z0);
        if (ox > TOL && oz > TOL) note('ERR', `${fid(f)}overlaps the ${nm}`);
      }
    }
    for (let i = 0; i < data.furniture.length; i++) for (let j = i + 1; j < data.furniture.length; j++) {
      const A = data.furniture[i], B = data.furniture[j];
      const ox = Math.min(A.x1, B.x1) - Math.max(A.x0, B.x0);
      const oz = Math.min(A.z1, B.z1) - Math.max(A.z0, B.z0);
      const oy = Math.min((A.y0 ?? 0) + A.h, (B.y0 ?? 0) + B.h) - Math.max(A.y0 ?? 0, B.y0 ?? 0);
      if (ox > TOL && oz > TOL && oy > 0.01) note('ERR', `${fid(A)}${fid(B)}furniture pieces overlap ${ox.toFixed(2)}×${oz.toFixed(2)} m`);
    }
    for (const w of allWalls) {
      const ax = axisOf(w), r = wallRect(w);
      const ext = data.extWalls.includes(w);
      for (const o of w.openings || []) {
        const from = (ax === 'x' ? r.x0 : r.z0) + o.at;
        const isDoor = ext ? o.door : (o.h >= 1.9 && (o.sill || 0) <= 0.1);
        if (isDoor) {
          const zone = ax === 'x'
            ? { x0: from, x1: from + o.w, z0: r.z0 - 0.7, z1: r.z1 + 0.7 }
            : { x0: r.x0 - 0.7, x1: r.x1 + 0.7, z0: from, z1: from + o.w };
          ...
          const span = ax === 'x'
            ? { x0: from, x1: from + o.w, z0: r.z0 - 0.8, z1: r.z1 + 0.8 }
            : { x0: r.x0 - 0.8, x1: r.x1 + 0.8, z0: from, z1: from + o.w };
          for (const f of data.furniture) {
            const ox = Math.min(span.x1, f.x1) - Math.max(span.x0, f.x0);
            const oz = Math.min(span.z1, f.z1) - Math.max(span.z0, f.z0);
            if (ox > 0.05 && oz > 0.05 && topOf(f) > o.sill + 0.1 && (f.y0 ?? 0) < o.sill + o.h)
              note('WARN', `${fid(f)}rises above the ${wid(w)}window sill (${o.sill} m)`);
          }
        }
      }
    }
  ...
  if (dxfPath) {
    const raw = require(resolve(dxfPath));
    const H = 19.25;
    const dxfV = [], dxfH = [];
    ...
    // built-in joinery faces satisfy DXF coverage too — the DXF draws wardrobes/kitchen runs
    // as furniture lines, but never the other direction (a model wall face still needs a
    // DXF wall line, so furniture stays out of the `near` check above)
    for (const f of data.furniture || []) {
      if (f.kind !== 'cab' && f.kind !== 'fix') continue;
      modelFaces.V.push([f.x0, f.z0, f.z1], [f.x1, f.z0, f.z1]);
      modelFaces.H.push([f.z0, f.x0, f.x1], [f.z1, f.x0, f.x1]);
    }
  ...
  const errs = problems.filter(p => p.sev === 'ERR').length;
  for (const p of problems) console.log(`${p.sev.padEnd(4)} [${name}] ${p.msg}`);
  console.log(`[${name}] ${errs} errors, ${problems.filter(p => p.sev === 'WARN').length} warnings, ${problems.filter(p => p.sev === 'INFO').length} info\n`);
  return errs;
}

const fileExports = require(resolve(process.argv[2]));
let totalErrs = 0;
Object.entries(fileExports).forEach(([name, data], i) => {
  totalErrs += verify(data, name, i === 0 ? process.argv[3] : undefined);
});
