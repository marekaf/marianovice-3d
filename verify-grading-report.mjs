...
assert.deepEqual(facility.from,[2.3,1.988663694038809]);
assert.deepEqual(facility.to,[7.3,1.988663694038809]);
assert.equal(facility.value,'5,00 × 2,50 m');
assert.deepEqual(facilityDepth.from,[2.3,1.988663694038809]);
assert.deepEqual(facilityDepth.to,[2.3,4.488663694038809]);
for(const dimension of [facility,facilityDepth])assert(result.mapSVG.includes(`data-dimension="${dimension.id}"`));
for(const [id,width,depth] of [['sauna',2.8,2.5],['pergola',6,4]]) {
