...
assert(covers(9.67,14.72),'Interior entrance reveal retains vinyl');
assert(!covers(9.97,14.72),'Vinyl must not extend outside the entrance door');
for(const p of parts)if(p.position[1]+p.size[1]/2>13.97+1e-8&&p.position[1]-p.size[1]/2<15.35-1e-8)assert(p.position[0]+p.size[0]/2<=9.6825+1e-8);
const checked=[];
for(const wall of data.extWalls.filter(w=>['N','S','E','W'].includes(w.face))){
  ...
  for(const [index,opening]of wall.openings.entries()){
    ...
    const model=data.buildOpening(wall,opening,index),frame=model.parts.find(p=>p.name.endsWith(model.opening.kind==='entrance'?'_threshold':'_frame_sill'));
    ...
    assert(covers(...sample(inner-mid+inward*.01)),`${wall.id}:${index} indoor reveal retains its floor finish`);
    ...
    for(const part of parts){
      ...
      assert(inward*(weatherEdge-inner)>=-1e-8,`${wall.id}:${index} full reveal width terminates at the indoor frame edge`);
    }
