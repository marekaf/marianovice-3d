...
- adds `exposureBias` (EV, default 0) to every shot's own bias. Panoramas look at the windows in every direction, so interiors that read fine in the flat render clip in 360; start around -2.
