const GreenhouseModel = (() => {
  function build(garden, terrainPlane, options = {}) {
    const element = garden.elements.find(e => e.id === 'greenhouse');
    ...
    const floorHeight = element.meta?.grading?.finishedLevel ?? options.floorHeight ?? Math.max(...[x,x+w].flatMap(px => [y,y+d].map(py => sample(px,py)))) + 0.04;
